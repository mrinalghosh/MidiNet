    ...
    if args.is_train:
        ...
        for epoch in range(epochs):
            ...
            for i, (data, prev_data, chord) in enumerate(train_loader, 0):
                ...
                D, D_logits, fm = netD(
                    real_cpu, chord_cpu, batch_size, pitch_range)

                # loss
                d_loss_real = reduce_mean(sigmoid_cross_entropy_with_logits(
                ...
                fake = netG(noise, prev_data_cpu, chord_cpu,
                            batch_size, pitch_range)
                label.fill_(fake_label)
                D_, D_logits_, fm_ = netD(
                    fake.detach(), chord_cpu, batch_size, pitch_range)
                d_loss_fake = reduce_mean(sigmoid_cross_entropy_with_logits(
                    D_logits_, torch.zeros_like(D_)))
                ...
                D_, D_logits_, fm_ = netD(
                    fake, chord_cpu, batch_size, pitch_range)

                # loss
                g_loss0 = reduce_mean(sigmoid_cross_entropy_with_logits(
                    D_logits_, torch.ones_like(D_)))
                # Feature Matching
                features_from_g = reduce_mean_0(fm_)
                features_from_i = reduce_mean_0(fm)
                fm_g_loss1 = torch.mul(
                    l2_loss(features_from_g, features_from_i), 0.1)
                ...
                if i % 100 == 0:
                    ...
                    fake = netG(fixed_noise, prev_data_cpu,
                                chord_cpu, batch_size, pitch_range)
