...
class sample_generator(nn.Module):
    ...
    def forward(self, z, prev_x, y, batch_size, pitch_range):
        ...
        h0_prev = lrelu(batch_norm_2d_cpu(
            self.h0_prev(prev_x)), 0.2)  # [72, 16, 16, 1]
        h1_prev = lrelu(batch_norm_2d_cpu(
            self.h1_prev(h0_prev)), 0.2)  # [72, 16, 8, 1]
        h2_prev = lrelu(batch_norm_2d_cpu(
            self.h2_prev(h1_prev)), 0.2)  # [72, 16, 4, 1]
        # [72, 16, 2, 1])
        h3_prev = lrelu(batch_norm_2d_cpu(self.h3_prev(h2_prev)), 0.2)
...
class generator(nn.Module):
    ...
    def forward(self, z, prev_x, y, batch_size, pitch_range):
        ...
        # h3_prev = F.leaky_relu(self.batch_nor_256(self.h0_prev(prev_x)),0.2)
        h0_prev = lrelu(batch_norm_2d(self.h0_prev(prev_x)),
                        0.2)  # [72, 16, 16, 1]
        h1_prev = lrelu(batch_norm_2d(self.h1_prev(h0_prev)),
                        0.2)  # [72, 16, 8, 1]
        h2_prev = lrelu(batch_norm_2d(self.h2_prev(h1_prev)),
                        0.2)  # [72, 16, 4, 1]
        # [72, 16, 2, 1])
...
class discriminator(nn.Module):
    ...
    def forward(self, x, y, batch_size, pitch_range):
        ...
        # torch.Size([72, 77, 3, 1])
        h1 = lrelu(batch_norm_2d(self.h1_prev(h0)), 0.2)
