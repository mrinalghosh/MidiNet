...
def make_a_track(eight_bar_binarized, track_name='melody', instrument=0):
    track = Track(pianoroll=eight_bar_binarized,
                  program=instrument, is_drum=False, name=track_name)
    return track
...
def make_a_demo(track1, track2, song_idx):
    ...
    multitrack = Multitrack(
        tracks=[track1, track2], tempo=120.0, beat_resolution=4)
...
def main():
    ...
    for i in range(data.shape[0]):
        if i % 100 == 0:
            ...
            multitrack.write('file'+sample_name+'_instru:_' +
                             instrument+'_volume:'+'.mid')
